...
class Visualizer(imgui_window.ImguiWindow):
    ...
    def draw_frame(self):
        ...
        if 'image' in self.result:
            ...
            for point in self.drag_widget.feature_points:
                pos_x = self.pane_w + max_w / 2 + (point[0] - self.image_w//2) * zoom
                pos_y = max_h / 2 + (point[1] - self.image_h//2) * zoom
                gl_utils.draw_circle(center=np.array([pos_x, pos_y]), color=[0,1,0], radius=3 * rescale)
            ...
            for point in self.drag_widget.targets:
                ...
                gl_utils.draw_circle(center=np.array([pos_x, pos_y]), color=[0,0,1], radius=3 * rescale)
            
            for point in self.drag_widget.points:
                pos_x = self.pane_w + max_w / 2 + (point[1] - self.image_w//2) * zoom
                pos_y = max_h / 2 + (point[0] - self.image_h//2) * zoom
                gl_utils.draw_circle(center=np.array([pos_x, pos_y]), color=[1,0,0], radius=3 * rescale)
